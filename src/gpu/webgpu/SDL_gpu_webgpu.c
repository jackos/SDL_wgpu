...
// FIXME: Issues with the shader parser:
//
                                                                                       ...
                                                                                       WGPUShaderStage stage)
{
    ...
    while (token != NULL) {
        ...
        if (foundKeywordCount == 3) {
            ...
            if (group == -1) {
                SDL_LogError(SDL_LOG_CATEGORY_GPU, "Token has no associated bind group! Token: %s", token);
            } else {
    ...
    WebGPURenderer *renderer)
{
    ...
    // HACK: Being able to filter RGBA32 textures is an optional feature on WebGPU.
    // Therefore, we cannot automatically assume that it'll be supported. (E.g; Firefox doesn't support it at all.)
    // So, we have special shaders just for RGBA32 textures that disable filtering alltogether. (This is a bad solution.)
